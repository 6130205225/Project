...
<body>
    @include('navbar')
    @foreach ($searchOneAt as $searchOneAt)
        <h1 class="text-5xl font-bold text-center py-5">{{ $searchOneAt->temple_name }}</h1>
        <h1 class="text-2xl font-bold text-center py-5">{{ $searchOneAt->activity_name }}</h1>

        <div class="divider"></div>

            <div class="card card-compact m-auto p-auto bg-base-100 shadow-xl w-full h-full ">
                <figure>
                    <img class="w-w-4/12 h-72 " src="{{ $searchOneAt->activity_pic_url }}" alt="">
                </figure>
                ...
                <div>
                    <div class="text-center">
                        ...
                        <div class="grid gap-5 md:grid-cols-1 grid-cols-1 mx-10">
                            <p class="text-left py-5">{{ $searchOneAt->activity_description }}</p>
                        </div>
                    </div>
                </div>
            </div>
